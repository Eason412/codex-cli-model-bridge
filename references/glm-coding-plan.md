...
3. For a mainland key, set `ZAI_CODING_BASE_URL=https://open.bigmodel.cn/api/coding/paas/v4` on the Router process. On macOS that is usually a LaunchAgent env. On Windows, set it in the shell or process that starts Router. Preserve the existing Router-compatible Node runtime and body-size limits, then restart Router after approval.
4. Add the requested model, such as `glm-5.3`, in `~/.codex/codex-router/user-models.json` so a Router upgrade does not drop it. Preserve unrelated model entries.
5. Change model visibility in `model-picker.json` only when the user requests it; preserve existing visibility by default.
